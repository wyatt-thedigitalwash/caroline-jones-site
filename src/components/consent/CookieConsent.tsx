    ...
    <>
      ...
      {showBanner && !showModal && (
        <div className="fixed inset-x-0 bottom-0 z-[60] p-4 sm:left-auto sm:right-4 sm:max-w-[420px]">
          <div className="border border-text-body/40 bg-background-alt p-4 shadow-2xl sm:p-6">
        ...
        >
          <div className="relative max-h-[88vh] w-full max-w-[540px] overflow-y-auto border border-text-body/20 bg-background-alt p-6 shadow-2xl sm:p-8">
            ...
            <button
              ...
              className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full text-text-accent/80 transition-colors hover:bg-[#5D3635]/10 hover:text-text-accent"
            >
              ...
              {CATEGORY_META.map((cat) => (
                <li key={cat.key} className="flex items-start justify-between gap-4 py-4">
                  <div>
                    ...
                    <p className="mt-1 font-body text-xs leading-relaxed text-text-accent/80">
                      {cat.description}
                    </p>
                  </div>
                  {cat.locked ? (
                    <span className="mt-1 shrink-0 font-body text-xs uppercase tracking-wide text-text-accent/80">
                      Always on
                    </span>
                  ) : (
                    <button
                      ...
                      className={`mt-1 flex h-6 w-11 shrink-0 items-center rounded-full px-0.5 transition-colors ${
                        draft[cat.key] ? "bg-[#5D3635] justify-end" : "bg-[#5D3635]/25 justify-start"
                      }`}
                    >
                      <span
                        className={`h-5 w-5 rounded-full ${draft[cat.key] ? "bg-[#DDE2CD]" : "bg-[#5D3635]"}`}
                      />
                    </button>
                  )}
                </li>
            ...
            <div className="mt-6 min-h-[20px]">
              {dirty && (
                <p className="flex items-center gap-2 font-body text-xs text-text-accent">
                  <span className="inline-block h-2 w-2 shrink-0 rounded-full bg-[#5D3635]" />
            ...
            <div className="mt-2 flex flex-col gap-2">
              ...
              <p className="mt-1 text-center font-body text-[11px] text-text-accent/80">
