  ...
  switch (block.type) {
    case "h2":
      return (
        <h2
          id={block.id}
          className="mt-12 font-body text-[24px] md:text-[28px]"
    ...
    case "h3":
      return (
        <h3
          id={block.id}
          className={`mt-8 font-body text-[19px] md:text-[22px] ${block.u ? "underline" : ""}`}
          style={{ color: "var(--text-accent)", scrollMarginTop: "7rem" }}
        >
          {block.text}
        </h3>
...
export default function LegalArticle({
  ...
  return (
    <>
      <section data-bg="primary" className="legal-typeset bg-background-primary px-5 pt-28 pb-4 lg:px-12">
        <div className="mx-auto max-w-[820px]">
          <Link href="/legal" className="font-body text-sm text-text-accent/70 hover:text-text-header">
            &larr; All Policies
          </Link>
          <h1 className="mt-6 font-body text-[30px] md:text-[44px]" style={{ color: "var(--text-accent)" }}>
            {title}
          </h1>
